...
Golang:
1. Golang is a compiled language. Golang compiles everything and the runtime is baked into the final product. So as long as you have `go` insatlled you dont need any other thing to run the go code.
...
Variables:
1. In go, if you have created a variable and havenot used it. You will get an error.
...
6.  `go get` command will add a new line like `require github.com/<package-name> <version>` stating the module used and its version. If you see //indirect at the end of <package-name>, it means we are not using the module is not being directly used.
...
2. Difference between thread and goroutines:
    2.1 Threads are managed by OS and goroutines are managed by Go runtime.
    2.2 Threads have fixed stack (1 MB) while goroutines have flexible stack.
